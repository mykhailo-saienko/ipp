...
public class TestIterable {
    ...
    private static void testNestedFiltered() {
        ...
        Iterator<Map.Entry<String, Integer>> it
                = new NestedIterator<>(i.iterator(), m -> m.entrySet().iterator());
    ...
    private BiIterable<String, Integer> createNested(Map<String, List<Integer>> source) {
        return () -> new NestedIterator<>(source.entrySet().iterator(),
                                          e -> new MappedIterator<>(e.getValue()
                                                  .iterator(), i -> new KeyValue<>(e.getKey(), i)));
    }
    ...
    private BiIterable<String, Integer> createTestDistinct() {
        List<Entry<String, Integer>> map
                = new ArrayList<>(asList(KVP("a", 1), KVP("b", 2), KVP("a", 3), KVP("c", 4)));
