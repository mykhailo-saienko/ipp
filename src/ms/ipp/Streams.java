...
/**
 * This class comprises a collection of most-widely-used queries on Streams, such as finding the
 * first element with given conditions (and automatically checking that the selected element is the
 * only one with those conditions), checking if none, any or all elements satisfy a condition or
 * re-packing elements into a different type of {@code Collection}.
 * 
...
public class Streams {
    ...
    /**
     * Returns true if none of the elements in the stream satisfy a given {@code Predicate}
     * 
     * @param st   the original stream, not null
    ...
    public static <T> T unique(Stream<T> st, Predicate<? super T> pred) {
        ...
        return filtered(st, pred).reduce((i1, i2) -> {
            throw new IllegalArgumentException("At least two elements encountered: [" + i1 + ", "
                                               + i2 + "]");
        }).orElse(null);
    }

    /**
     * Returns the first non-null element encountered in the stream which satisfies a given
     * {@code Predicate}. If null is returned, it means that no elements satisfying this predicate
     * exist.
     * 
     * @param st   the original stream, not null
     * @param pred the predicate. This predicate must never return true for null elements otherwise
     *             {@code NullPointerException} may be thrown. null-Predicate is interpreted as a
     *             condition which always returns true (and hence itself is not safe/should be
     *             avoided if stream contains null values).
     * @return
     */
     ...
     */
    public static <T> void forEach(Stream<T> st,
                                   Predicate<? super T> pred,
                                   Consumer<? super T> proc) {
        if (st != null) {
    ...
    /**
     * Collects all elements from the stream which satisfy a given {@code Predicate} by using a
     * given {@link Collector}.
     * 
     * @param st        the original stream, not null
     * @param pred      the predicate. If null, the original stream is used
     * @param collector the Collector, as it is used by @{code Stream::collect} methods
     */
    public static <T, U extends T, R> R collect(Stream<U> it,
    ...
    /**
     * Collects at most the first {@code n} elements from the stream which satisfy a given
     ...
     */
    public static <T, U extends T, R> R collect(Stream<U> st,
                                                Predicate<? super U> pred,
                                                Collector<T, ?, R> collector,
                                                int n) {
        if (st == null) {
    ...
    /**
     * Collects all elements from the stream which satisfy a given {@code Predicate} to a
     * {@code List}.
     * 
     * @param st   the original stream, not null
     * @param pred the predicate. If null, the original stream is used
    ...
    /**
     * Returns a {@code Stream} which filters out elements that don't satisfy a given
     * {@code Predicate}.
     * 
     * @param st   the original stream, not null.
    ...
    /**
     * Returns a {@code Stream} which maps elements by means of a given {@code Function}.
     * 
     * @param st  the original stream, not null
    ...
    public static <T> Stream<T> stream(Iterable<T> it) {
        ...
        return it instanceof Collection ? ((Collection<T>) it).stream()
